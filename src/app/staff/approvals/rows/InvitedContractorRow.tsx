...
export default function InvitedContractorRow({
  inviteDetails,
  index,
  user,
  setInviteToArchiveObject,
  activeFilter,
  removeInviteFromExpired,
}: any) {
  const inviteHasExpired = () => {
    const currentDate = new Date();
    const expiryDate = inviteDetails?.expiry?._seconds
      ? new Date(inviteDetails.expiry._seconds * 1000)
      : new Date(inviteDetails.expiry);
    return currentDate.getTime() > expiryDate.getTime();
  };
  const hasCnPPermissions = () =>
    ["Admin", "HOD", "VRM", "CnP Staff", "Supervisor"].includes(user.role);
  ...
  const recommended = getRecommendedBy();

  return (
    <tr className={index % 2 === 0 && (styles.rowDarkBackground as any)}>
      <td>
        <p className={styles.contractorName}>
          {String(inviteDetails.companyName).toLocaleUpperCase()}
        </p>
      </td>
      <td className={styles.userDetails}>
        <p>{`${inviteDetails.fname} ${inviteDetails.lname}`.toLocaleUpperCase()}</p>
        <p>
          {inviteDetails.vendorAppAdminProfile?.email
            ? inviteDetails.vendorAppAdminProfile?.email
            : inviteDetails.email}
        </p>
      ...
      <td className={styles.status}>
        ...
        {inviteHasExpired() && !inviteDetails.used && (
          <>
            ...
            <p className={styles.statusDateText}>
              Expired: {getDateFromDateString(inviteDetails.expiry)}
            </p>
            <div className={styles.renewRequestTextDiv}>
              {renewText !== "EXTENDED EXPIRY DATE" && hasCnPPermissions() && (
                <a className={styles.renewText} onClick={renewRequest}>
                  {renewText}
                </a>
              )}
              {renewText === "EXTENDING EXPIRY DATE..." && <PrimaryColorSmallLoadingIcon />}
              {renewText === "EXTENDED EXPIRY DATE" && (
                <p className={styles.reminderSentText}>EXPIRY DATE EXTENDED</p>
              )}
            </div>
          </>
        )}
        ...
        {inviteDetails.used && (
          <>
            ...
            <p className={styles.statusDateText}>
              Sent: {getDateFromDateString(inviteDetails.timeApproved)}
            </p>
            <p className={styles.statusDateText}>
              Used: {getDateFromTimestamp(inviteDetails.used)}
            </p>
          </>
        )}
          ...
          <>
            ...
            <p className={styles.statusDateText}>
              ...
              {getDateFromDateString(
                inviteDetails.timeApproved ? inviteDetails.timeApproved : inviteDetails.createdAt,
              )}
            </p>
            {inviteDetails.lastReminderSent && (
              <p className={styles.statusDateText}>
                Last Reminder Sent: {getDateFromDateString(inviteDetails.lastReminderSent)}
              </p>
            )}
            {sendReminderText !== "REMINDER SENT" && (
              <div className={styles.reminderDiv}>
                <a onClick={sendReminder} className={styles.renewText}>
                  {sendReminderText}
                </a>
            ...
            {sendReminderText === "REMINDER SENT" && (
              <p className={styles.reminderSentText}>REMINDER SENT</p>
            )}
            {hasCnPPermissions() && (
        ...
        {(user.role === "Admin" || user.role === "HOD" || user.role === "VRM") &&
          !inviteDetails.used && (
            <p
              className={styles.deleteInviteText}
              onClick={() => setInviteToArchiveObject(inviteDetails)}
            >
              Archive Invite
            </p>
          )}

...
export default function L3Row({index, companyRecord, revertToL2, user}:any){
  ...
  return (
    <tr className={[styles.l3Item, index%2===0 && styles.rowDarkBackground].join(" ")}>
      <td><Link href={`/staff/vendor/${companyRecord._id}`}>{String(companyRecord.companyName).toUpperCase()}</Link><p>{String(companyRecord?.vendorAppAdminProfile?.email ? companyRecord?.vendorAppAdminProfile?.email : companyRecord?.contractorDetails?.email)}</p></td>
      <td>{hasAdminPermissions(user.role) && <a onClick={()=> revertToL2(companyRecord.vendor)}>MOVE TO L2</a>}</td>

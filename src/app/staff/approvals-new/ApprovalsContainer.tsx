...
export default function ApprovalsContainer() {
  ...
  const [approvals, setApprovals] = useState<any>({
    completedL2: [], inProgress: [], invites: [], l3: [], pendingL2: [], returned: [], parkRequested: []
  })
  ...
  const updateTabLabelsWithCounts = (counts: any) => {
    setApprovalsTabs(prev =>
      prev.map(tab => {
        let count = 0
        switch (tab.name) {
          case 'invited': count = counts.invites?.total || 0; break
          case 'in-progress': count = counts.inProgress || 0; break
          case 'pending-l2': count = counts.pendingL2 || 0; break
          case 'l3': count = counts.l3 || 0; break
          case 'completed-l2': count = counts.completedL2 || 0; break
          case 'returned': count = counts.returned || 0; break
          case 'park-requests': count = counts.parkRequested || 0; break
        }
        return { ...tab, label: count > 0 ? `${tab.label.split(' (')[0]} (${count})` : tab.label.split(' (')[0] }
      })
    )
  }
  ...
  const getActiveTable = () => {
    switch (activeTab) {
      case "invited": return tableHeaders.invited
      case "in-progress": return tableHeaders.inProgress
      case "pending-l2": return tableHeaders.pendingL2
      case "l3": return tableHeaders.l3
      case "completed-l2": return tableHeaders.completedL2
      case "park-requests": return tableHeaders.parkRequests
      default: return tableHeaders.returned
    }
  }
  ...
  const filterInvitedCompaniesByNameOrEmail = (name: string) => {
    const temp = { ...approvals }
    temp.invites = fixedApprovals.invites.filter((item: any) =>
  ...
  const [archiveStatusMessages, setArchiveStatusMessages] = useState({ errorMessage: "", successMessage: "" })
  ...
  const archiveInvite = async () => {
    try {
      ...
      try {
        ...
        setArchivingInvite(false)
        setArchiveStatusMessages({ errorMessage: "", successMessage: "Invite archived successfully." })
      } catch (rtkError) {
  ...
  const searchVendors = async (query: string) => {
    ...
    switch (currentSearchFilter) {
      ...
      case "pending": filterParam = "pending"; break
      case "parked": filterParam = "parked"; break
      case "l3": filterParam = "l3"; break
      case "returned": filterParam = "returned"; break
  ...
  const removeFIP = () => { setTimeout(() => setActionProgress(""), 4000) }
  ...
  const showSortIcons = (index: number) => {
    ...
    if (activeTab === "pending-l2" || activeTab === "completed-l2" || activeTab === "returned" || activeTab === "park-requests") return (index === 0 || index === 3)
  ...
  const toggleNameSort = () => {
    ...
    if (nameSortAscending) {
      ...
      else if (activeTab === "completed-l2") temp.completedL2 = sortArrayByNameDescending(temp.completedL2)
      else if (activeTab === "in-progress") temp.inProgress = sortArrayByNameDescending(temp.inProgress)
  ...
  const sortArrayNumerically = (array: any[]) => array.sort((a, b) => {
    const extract = (x: any) => {
  ...
  const sortArrayNumericallyDescending = (array: any[]) => array.sort((a, b) => {
    const extract = (x: any) => {
      if (x.lastUpdate) return new Date(x.lastUpdate._seconds * 1000).getTime()
      if (x.lastApproved) return new Date(x.lastApproved).getTime()
      if (x.approvalActivityHistory) return new Date(x.approvalActivityHistory[0].date).getTime()
  ...
  const toggleDateSort = () => {
    let temp: any = (currentSort === "numerical") ? { ...approvals } : { ...fixedApprovals }
    if (dateSortAscending) {
      if (activeTab === "pending-l2") temp.pendingL2 = sortArrayNumericallyDescending(temp.pendingL2)
      else if (activeTab === "completed-l2") temp.completedL2 = sortArrayNumericallyDescending(temp.completedL2)
      else if (activeTab === "in-progress") temp.inProgress = sortArrayNumericallyDescending(temp.inProgress)
      else if (activeTab === "l3") temp.l3 = sortArrayNumericallyDescending(temp.l3)
      else if (activeTab === "returned") temp.returned = sortArrayNumericallyDescending(temp.returned)
    } else {
      if (activeTab === "pending-l2") temp.pendingL2 = sortArrayNumerically(temp.pendingL2)
      else if (activeTab === "completed-l2") temp.completedL2 = sortArrayNumerically(temp.completedL2)
  ...
  const userIsCnPStaff = () => (user?.role === "C and P Staff" || user?.role === "Admin" || user?.role === "C&P Admin" || user?.role === "IT Admin" || user?.role === "Supervisor" || user?.role === "VRM" || user?.role === "HOD")

  const [exportOptions, setExportOptions] = useState<any>({
    root: "invited", selectedInviteType: "all", selectedStages: [], l2Stages: [], pendingL2Stages: [], exportType: "all", selectedVendors: []
  })
  ...
  const updateExportOptions = (option: string, value: any) => setExportOptions((p: any) => ({ ...p, [option]: value }))
  const toggleExportOptions = (option: string, value: any) => {
    setExportOptions((p: any) => {
      const has = p[option].includes(value)
      return { ...p, [option]: has ? p[option].filter((x: any) => x !== value) : [...p[option], value] }
    })
  }
  ...
  const findVendorsByName = (vendorName: string) => {
    ...
    if (exportOptions.selectedStages.includes("inProgress")) {
      fixedApprovals.inProgress?.forEach((item: any) => {
        if (String(item.companyName).toLowerCase().includes(String(vendorName).toLowerCase())) push({ ...item, stage: "In Progress" })
      })
    }
    if (exportOptions.selectedStages.includes("l2")) {
        ...
        fixedApprovals.pendingL2?.forEach((item: any) => {
          if (String(item.companyName).toLowerCase().includes(String(vendorName).toLowerCase())) {
            if (item.flags.status === "pending" || item.flags.stage === "pending") {
              if (exportOptions.pendingL2Stages.includes("All")) push({ ...item, l2Stage: "Pending", stage: "L2", l2PendingStage: getL2PendingStage(item.flags) })
              else {
                const L = item.flags?.approvals?.level
                if (exportOptions.pendingL2Stages.includes("A") && (!L)) push({ ...item, l2PendingStage: "A", l2Stage: "Pending", stage: "L2" })
                if (exportOptions.pendingL2Stages.includes("B") && (L === 1)) push({ ...item, l2PendingStage: "B", l2Stage: "Pending", stage: "L2" })
                if (exportOptions.pendingL2Stages.includes("C") && (L === 2)) push({ ...item, l2PendingStage: "C", l2Stage: "Pending", stage: "L2" })
                if (exportOptions.pendingL2Stages.includes("D") && (L === 3)) push({ ...item, l2PendingStage: "D", l2Stage: "Pending", stage: "L2" })
                if (exportOptions.pendingL2Stages.includes("E") && (L === 4)) push({ ...item, l2PendingStage: "E", l2Stage: "Pending", stage: "L2" })
                if (exportOptions.pendingL2Stages.includes("F") && (L === 5)) push({ ...item, l2PendingStage: "F", l2Stage: "Pending", stage: "L2" })
                if (exportOptions.pendingL2Stages.includes("G") && (L === 6)) push({ ...item, l2PendingStage: "G", l2Stage: "Pending", stage: "L2" })
                if (exportOptions.pendingL2Stages.includes("H") && (L === 7)) push({ ...item, l2PendingStage: "H", l2Stage: "Pending", stage: "L2" })
              }
            }
          }
        })
      ...
      if (exportOptions.l2Stages.includes("completed")) {
        fixedApprovals.completedL2?.forEach((item: any) => {
          if (String(item.companyName).toLowerCase().includes(String(vendorName).toLowerCase())) {
            if (item.flags.status === "parked" || item.flags.stage === "parked") push({ ...item, l2Stage: "Completed", stage: "L2" })
          }
        })
      }
      if (exportOptions.l2Stages.includes("returned")) {
        fixedApprovals.returned?.forEach((item: any) => {
          if (String(item.companyName).toLowerCase().includes(String(vendorName).toLowerCase())) {
            if (item.flags.status === "returned" || item.flags.stage === "returned") push({ ...item, l2Stage: "Returned", stage: "L2" })
          }
        })
      }
      if (exportOptions.l2Stages.includes("returnRequested") && fixedApprovals.returnRequested) {
        fixedApprovals.returnRequested?.forEach((item: any) => {
          if (String(item.companyName).toLowerCase().includes(String(vendorName).toLowerCase())) {
            if (item.flags.status === "park requested" || item.flags.stage === "park requested") push({ ...item, l2Stage: "Return Requested", stage: "L2" })
          }
        })
      }
    }
    if (exportOptions.selectedStages.includes("l3")) {
      fixedApprovals.l3?.forEach((item: any) => {
        if (String(item.companyName).toLowerCase().includes(String(vendorName).toLowerCase())) push({ ...item, stage: "L3" })
      })
    }
  ...
  const exportRegisteredVendorsToExcel = (exportData: any[]) => {
    ...
    const settings: any = { fileName: "VendorListExport", extraLength: 3, writeMode: "writeFile", writeOptions: {}, RTL: false }
    xlsx(data, settings)
  }

  const inviteIsActive = (invite: any) => {
    const currentDate = new Date()
    const expiryDate = invite?.expiry?._seconds ? new Date(invite.expiry._seconds * 1000) : new Date(invite.expiry)
  ...
  const exportExcelFile = (exportData: any[]) => {
    const data: any = [{
      ...
      columns: [
        ...
        { label: "Status", value: (row: any) => row.used ? "Used" : (inviteIsActive(row) ? "Active" : (inviteHasExpired(row) ? "Expired" : "")) }
      ],
      content: exportData,
    }]
    const settings: any = { fileName: "VendorListExport", extraLength: 3, writeMode: "writeFile", writeOptions: {}, RTL: false }
  ...
  const closeExportModal = () => {
    setExportOptions({ root: "invited", selectedInviteType: "all", selectedStages: [], l2Stages: [], pendingL2Stages: [], exportType: "all", selectedVendors: [] })
    setVendorsToExport([])
  ...
  return (
    <div className={styles.approvals}>
      ...
      {returnToL2Data && <RevertToL2Modal
      ...
      {Object.values(inviteToArchive)?.length > 0 && <ArchiveInviteModal
      ...
      {!fetchingContractors && (
        <>
          ...
          {currentTabLoading ? (
            ...
            >
              {activeTab === "invited" && approvals.invites.map((item: any, index: number) => (

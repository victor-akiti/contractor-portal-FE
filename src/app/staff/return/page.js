...
const Approvals = () => {
  ...
  const fetchAllApprovalsData = async () => {
    try {
      ...
      if (fetchAllApprovalsDataRequest.status === "OK") {
        ...
        if (
          fetchAllApprovalsDataRequest?.data?.parkRequested &&
          fetchAllApprovalsDataRequest?.data?.parkRequested?.length > 0 &&
          (user.role === "Admin" ||
            user.role === "HOD" ||
  ...
  const filterL2Companies = (stage) => {
    ...
    if (stage === "All") {
      ...
      tempApprovals.pendingl2 = fixedApprovals.pendingL2.filter(
        (item) => !item?.flags?.approvals?.level,
      );
  ...
  const filterInvitedCompaniesByNameOrEmail = (name) => {
    setActiveFilter("");
    let tempApprovals = { ...approvals };
    tempApprovals.invites = fixedApprovals.invites.filter(
      (item) =>
        String(item.companyName).toLocaleLowerCase().includes(String(name).toLocaleLowerCase()) ||
        String(item.email).toLocaleLowerCase().includes(String(name).toLocaleLowerCase()),
    );
    setApprovals(tempApprovals);
  };
  ...
  const showSortIcons = (index) => {
    if (activeTab === "invited") {
      ...
      if (activeTab === "in-progress") {
        ...
        activeTab === "returned-to-contractor" ||
        activeTab === "parkRequests"
      ) {
        if (index === 1 || index === 4) {
          return true;
        } else {
  ...
  const returnApplications = async () => {
    try {
      ...
      const returnApplicationsRequest = await postProtected(
        "approvals/existing/return",
        { selectedVendors },
        user.role,
      );
  ...
  return (
    <div className={styles.approvals}>
          ...
          <div className={styles.returnApplicationsModal}>
            ...
            <div>
              <p className={styles.modalBodyText}>
                You are about to return the following vendors applications. After re-submission,
                their applications will be returned to stage A.
              </p>

              <div>
                {selectedVendorsData.map((item, index) => (
                  <p key={index} className={styles.companyItem}>
                    {item.companyName}
                  </p>
                ))}
              </div>
            ...
            {returnApplicationStatus.status === "success" && (
              <SuccessMessage message={"Vendor applications returned."} />
            )}

            {returnApplicationStatus.status === "error" && (
              <ErrorText text={returnApplicationStatus.message} />
            )}

            <div className={styles.footer}>
              {returnApplicationStatus.status !== "success" && (
                <button onClick={() => returnApplications()}>
                  Continue {returnApplicationStatus.status === "returning" && <ButtonLoadingIcon />}
                </button>
              )}
              <button onClick={() => closeReturnApplicationsModal()}>
                {returnApplicationStatus.status === "success" ? "Close" : "Cancel"}
              </button>
            </div>
          </div>
        </Modal>
          ...
          <>
            ...
            <div className={styles.searchFilterDiv}>
              ...
              {searchQueryResults.length > 0 && (
                <div className={styles.searchResultsDiv} ref={searchResultRef}>
                  {searchQueryResults.map((item, index) => (
      ...
      <div className={styles.stageAReturnButtonDiv}>
        <button
          className={selectedVendors.length === 0 ? styles.inactive : styles.active}
          onClick={() => {
        ...
        <button onClick={() => clearSelectedVendors()} className={styles.clearButton}>
          Clear Selected List
        </button>
      </div>
      ...
      {!fetchingContractors && (
        <>
          ...
          <div className={styles.inviteFilters}>
            ...
            {activeTab === "invited" && (
              <div>
                {inviteFilters.map((item, index) => (
                  <p
                    className={item === activeFilter && styles.active}
                    key={index}
                    onClick={() => {
                ...
                <input
                  ...
                  onChange={(event) => filterInvitedCompaniesByNameOrEmail(event.target.value)}
                />
              </div>
            )}
            ...
            {activeTab === "l3" && (
              <div>
                {l3Filters.map((item, index) => (
                  <p
                    className={item === activeL3Filter && styles.active}
                    key={index}
                    onClick={() => {
          ...
          <table>
            <thead>
              {getActiveTable().map((item, index) => (
                <td key={index}>
                  <div className={styles.tableHeading} onClick={() => getSortToPerform(index)}>
                    {showSortIcons(index) && (
                      <Image src={getIconToDisplay(index)} alt="sort icon" width={15} height={15} />
                    )}
                    <p>{item}</p>
                  </div>
                </td>
            ...
            <tbody>
              {activeTab === "pending-l2" &&
                approvals.pendingL2.map((item, index) => (
                  <PendingL2Item
                    key={index}
                    user={user}
                    companyRecord={item}
                    ...
                    toggleVendorSelection={(vendorData) => toggleVendorSelection(vendorData)}
                  />
                ))}

              {activeTab === "completed-l2" &&
                approvals.completedL2.map((item, index) => (
                  <CompletedL2Item
                    revertToL2={(vendorID) => setDataForReturnToL2(item._id, "parked")}
                    key={index}
                    user={user}
                    companyRecord={item}
                    index={index}
              ...
              {activeTab === "returned-to-contractor" &&
                approvals.returned.map((item, index) => (
                  <ReturnedItem
                    key={index}
                    user={user}
                    companyRecord={item}
...
const PendingL2Item = ({ index, companyRecord, user, selectedVendors, toggleVendorSelection }) => {
  const userCanViewActions = () => {
    if (user.role === "Admin" || user.role === "HOD" || user.role === "Executive Approver") {
      ...
      companyRecord?.flags?.level === 2 &&
      companyRecord.currentEndUsers.includes(user._id)
    ) {
      return true;
    } else if (
      user.role === "VRM" &&
      (!companyRecord?.flags?.level || companyRecord?.flags?.level === 3)
    ) {
      return true;
    } else if (
      user.role === "CO" &&
      (!companyRecord?.flags?.level || companyRecord?.flags?.level === 2)
    ) {
      return true;
    } else if (
  ...
  return (
    <tr
      className={[
        selectedVendors.includes(companyRecord._id) && styles.selectedVendor,
    ...
    >
      <td>
        <input
          checked={selectedVendors.includes(companyRecord._id)}
          type="checkbox"
          onClick={() => toggleVendorSelection(companyRecord)}
        />
      </td>
      <td>
        <Link href={`/staff/vendor/${companyRecord._id}`}>
          {String(companyRecord.companyName).toLocaleUpperCase()}
        </Link>
        {/* <p>
